 ...
 */
const restoreFormService = async datauser => {
    const cxauthxc = datauser.cxauthxc;
    const restoreServerURL = datauser.restoreServerURL;
    delete(datauser.restoreServerURL);
    ...
    try {
        const { data } = await axios.post(restoreServerURL, datasend, config);
        if(import.meta.env.REACT_APP_SHOW_CONSOLE === 'true'){
            console.log('%c [Conexys] [Request] ', 'color: #fc0000', restoreServerURL);
        };
        return data;
    } catch (error) {
