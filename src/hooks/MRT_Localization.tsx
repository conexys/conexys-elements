...
import type {
  MRTLocalizationReturn,
  MRTLocalizationText,
} from '../types/hooks/mrt_Localization.types';

/**
 * React component for Material React Table (MRT) localization.
 *
 * @returns {MRTLocalizationReturn} An object containing localized text for MRT actions and messages.
 */
const MRT_Localization = (): MRTLocalizationReturn => {
  const [t] = useTranslation('materialreacttable');

  const textMRT: MRTLocalizationText = {
    actions: t('actions'),
    and: t('and'),
    cancel: t('cancel'),
