 ...
 */
const AppHeaderPage404: React.FC<AppHeaderPage404Props> = ({
  title,
  fpHash,
}) => {
  ...
  useEffect(() => {
    ...
    if (themeElement) {
      if (menuRigth) {
        themeElement.classList.add('sidebar-right-opened');
      } else {
        themeElement.classList.remove('sidebar-right-opened');
      }
    }
  }, [menuRigth]);
