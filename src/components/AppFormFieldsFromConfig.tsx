...
import type {
  FormBlockComponents,
  FormConfig,
  AppFormFieldsFromConfigProps,
} from '../types/components/appFormFieldsFromConfig.types';
import type { FormInputs } from '../types/common';

/**
 * Component for rendering form fields based on a provided configuration.
 * @param {AppFormFieldsFromConfigProps} props - Component properties.
 * @param {FormInputs} props.formInputs - Form input values.
 ...
 * @param {string} props.nouser - Specifies if the form is for a user with no authentication.
 * @param {'post'|'patch'} [props.method='post'] - HTTP method for form submission.
 * @param {'post'|'patch'} [props.restoreMethod='post'] - HTTP method for restore operations.
...
export default function AppFormFieldsFromConfig({
  ...
  permissionstatus = 2,
  nouser,
  t: externalTranslation,
  fetchMethod = 'post',
  method = 'post',
  restoreMethod = 'post',
}: AppFormFieldsFromConfigProps): React.JSX.Element | null {
  const [defaultT] = useTranslation('global');

  const t: ReturnType<typeof useTranslation>[0] =
    externalTranslation || defaultT;

  const useServerForm: boolean = Boolean(
    fpHash && postServerURL && getServerURL && deleteServerURL,
  );

  // If server URLs are provided, use useCustomForm hook, otherwise use external inputs and handlers
  const {
    formInputs: serverFormInputs,
    handleInputChange: serverHandleInputChange,
  } = useServerForm
    ? useCustomForm(
        fpHash,
        postServerURL,
  ...
  const formInputs: FormInputs = useServerForm
    ? serverFormInputs
    : externalFormInputs || {};
  const handleInputChange: (event: any) => void = useServerForm
    ? serverHandleInputChange
  ...
  return (
    <>
      {formConfig.content.body.map((block: FormBlockComponents) =>
        RenderForm(
          {
            ...
            onCreateNew: block.onCreateNew,
            allowCreate: block.allowCreate,
            style: block.style,
